...
describe("timer core", () => {
  it("creates the classic 25 minute focus timer", () => {
    const state = createDefaultState(new Date(2026, 7, 7));
  ...
  it("calculates remaining time from the absolute deadline", () => {
    const state = { ...createDefaultState(), status: "running" as const, endAt: 50_000 };
    assert.equal(getRemainingMs(state, 20_000), 30_000);
    assert.equal(getRemainingMs(state, 60_000), 0);
  });

  it("offers a long break after the configured focus count", () => {
    const state = { ...createDefaultState(), cycleFocusCount: 3 };
    const result = completeInterval(state, new Date());
  ...
  it("returns to focus after a break without incrementing stats", () => {
    ...
    const state = { ...initial, mode: "shortBreak" as const, todayCount: 2 };
  ...
  it("resets only the daily counter on a new local date", () => {
    const state = { ...createDefaultState(new Date(2026, 7, 7)), todayCount: 5, cycleFocusCount: 2 };
    const normalized = normalizeDay(state, new Date(2026, 7, 8));
  ...
  it("clamps settings and uses them for durations", () => {
    ...
    assert.equal(durationMs("longBreak", settings), 40 * 60_000);
  });
});
