...
function badgeText(state: TimerState, now = Date.now()): string {
...
async function syncBadge(state: TimerState): Promise<void> {
  await chrome.action.setBadgeText({ text: badgeText(state) });
  await chrome.action.setBadgeBackgroundColor({
    color: state.status === "paused" ? "#737987" : badgeColors[state.mode]
  });
}

async function scheduleAlarms(state: TimerState): Promise<void> {
  await Promise.all([
    chrome.alarms.clear(END_ALARM),
    chrome.alarms.clear(BADGE_ALARM)
  ]);
  if (state.status !== "running" || state.endAt === null) return;
  chrome.alarms.create(END_ALARM, { when: state.endAt });
...
async function notifyCompletion(mode: TimerMode): Promise<void> {
...
async function reconcileState(notify: boolean): Promise<TimerState> {
  ...
  if (
    ...
    const completed = completeInterval(state);
    state = completed.state;
    await saveState(state);
    await scheduleAlarms(state);
    if (notify) await notifyCompletion(completed.completedMode);
  }
  await syncBadge(state);
  return state;
}

async function handleCommand(command: TimerCommand): Promise<TimerState> {
  ...
  switch (command.type) {
    ...
    case "RESET":
      state = {
        ...
        status: "idle",
        endAt: null,
        remainingMs: durationMs(state.mode, state.settings)
      };
      break;
    case "SAVE_SETTINGS": {
      if (state.status !== "idle") {
        throw new Error("Настройки можно менять только при остановленном таймере");
      }
      const settings = validateSettings(command.settings);
      state = {
        ...state,
        settings,
        remainingMs: durationMs(state.mode, settings)
      };
      break;
    }
  }
...
chrome.runtime.onMessage.addListener(
  (command: TimerCommand, _sender, sendResponse: (response: TimerResponse) => void) => {
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === END_ALARM) {
    void queueMutation(() => reconcileState(true));
  }
  if (alarm.name === BADGE_ALARM) {
    void queueMutation(async () => {
      const state = await reconcileState(false);
      await scheduleAlarms(state);
      return state;
    });
  }
...
chrome.runtime.onStartup.addListener(() => {
  void queueMutation(() => reconcileState(true).then(async (state) => {
    await scheduleAlarms(state);
  }));
});
