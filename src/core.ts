...
export function durationMs(mode: TimerMode, settings: Settings): number {
  const minutes = {
...
export function createDefaultState(now = new Date()): TimerState {
  return {
    mode: "focus",
    status: "idle",
    endAt: null,
...
export function normalizeDay(state: TimerState, now = new Date()): TimerState {
  const today = localDateKey(now);
  return state.statsDate === today
    ? state
    : { ...state, statsDate: today, todayCount: 0 };
}
...
export function completeInterval(
  input: TimerState,
  now = new Date()
): { state: TimerState; completedMode: TimerMode } {
  ...
  if (completedMode === "focus") {
    ...
    nextMode =
      cycleFocusCount >= state.settings.sessionsBeforeLongBreak
        ? "longBreak"
        : "shortBreak";
    if (nextMode === "longBreak") cycleFocusCount = 0;
  }
...
export function validateSettings(settings: Settings): Settings {
  const integer = (value: number, min: number, max: number) => {
    if (!Number.isFinite(value)) throw new Error("Некорректное значение");
    return Math.min(max, Math.max(min, Math.round(value)));
  };
  return {
    focusMinutes: integer(settings.focusMinutes, 1, 180),
    shortBreakMinutes: integer(settings.shortBreakMinutes, 1, 60),
    longBreakMinutes: integer(settings.longBreakMinutes, 1, 120),
    sessionsBeforeLongBreak: integer(settings.sessionsBeforeLongBreak, 1, 12)
  };
}
